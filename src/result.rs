...
pub fn show_result( site_list : &Vec<String>, cons_re_list : &Vec<f64>, arg_c : &String )
{
	if ( *site_list ).len() != ( *cons_re_list ).len() { error::error_bomb( "site_ent_len_not_same" ); }
	...
	if ( *arg_c ).as_str() == "yes" {
		...
		*/
		...
		println!( "num\tent\tsite" );
		for i in 0 .. ( *cons_re_list ).len() {
			println!( "{}\t{:.3}\t{}", i + 1, ( *cons_re_list )[ i ], ( *site_list )[ i ] );
		}
	}
}

pub fn save_result( site_list : &Vec<String>, cons_re_list : &Vec<f64>, arg_o : &String )
{
	...
	for i in 0 .. ( *cons_re_list ).len() {
		writeln!( fout, "{}\t{:.3}\t{}", i + 1, ( *cons_re_list )[ i ], ( *site_list )[ i ] ).expect( "FAILED to write" );
	}
